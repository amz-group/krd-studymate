...
// Center panel. Owns all drag/resize interaction so coordinates stay in the
// slide's logical space (pointer deltas are divided by the view scale). One
// commit per gesture = one undo entry.
export default function EditorCanvas({
  slide, ratio, language, zoomMode, selectedIds, editingId, showGrid, snap,
  ...
  const onMove = (e) => {
    ...
    if (d.mode === 'move') {
      ...
      let g = { x: null, y: null };
      if (snap) {
        const peers = slide.elements.filter((el) => !d.orig[el.id]);
  ...
  const startDrag = (e, elId, mode, handle) => {
    ...
    const orig = {};
    ids.forEach((id) => {
      const x = slide.elements.find((e2) => e2.id === id);
    ...
    <div ref={ref} className="flex-1 min-w-0 overflow-auto bg-muted/40 p-3"
      ...
      <div className="min-h-full min-w-full flex items-center justify-center">
        <div style={{ width: baseW * scale, height: baseH * scale, boxShadow: '0 12px 40px rgba(0,0,0,0.18)', borderRadius: 4, overflow: 'hidden' }}>
          <div style={{ width: baseW, height: baseH, transform: `scale(${scale})`, transformOrigin: 'top left', position: 'relative', direction: isRtl(language) ? 'rtl' : 'ltr', ...bgCss(slide.background) }}>
